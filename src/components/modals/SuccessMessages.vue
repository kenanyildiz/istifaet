...
<template>
  <b-modal id="messagesModal" v-model="isDisplay"
    ...
    <b-container fluid>
      <b-row class="text-left">
        <b-col>
          ...
          <div v-if="contentType === 'success' && type === 'suggestion'" class="alert alert-success" role="alert">
            {{ content }}
          </div>
    ...
    <div slot="modal-footer" class="w-100 text-right">
      <b-btn variant="custom btn-outline-dark" data-dismiss="modal" @click.prevent="modalDisplayChanged($event, { name: 'messagesModalOpt', display: false, contentType: null })">
        {{ $t("close") }}
      </b-btn>
    </div>
  </b-modal>
